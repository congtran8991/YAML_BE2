...
class GroupDatasetPermissionModel(ModelBase):
    ...
    __table_args__ = (
        UniqueConstraint("group_dataset_id", "granted_to_user_id", name="uq_group_dataset_user"),
    )

    id = Column(Integer, primary_key=True, index=True)
    group_dataset_id = Column(Integer, ForeignKey("groupdatasets.id", ondelete="CASCADE"), nullable=False)
    granted_to_user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False) # user nhận quyền
    can_view = Column(Boolean, default=False)
    can_edit = Column(Boolean, default=False)
    can_delete = Column(Boolean, default=False)
    granted_by_user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)  # user cho quyền
    granted_at = Column(TIMESTAMP, default=datetime.now())

    # Quan hệ (optional - giúp dễ truy cập thông tin nếu cần)
    group_datasets = relationship("GroupDatasetModel", back_populates="permissions_group_datasets")
